...
export default function Properties() {
  ...
  const saveCurrentSearch = () => { const generatedLabel = [propertyType !== "All" ? propertyType : "Properties", municipality !== "All" ? municipality.replace(" Municipality", "") : location || "Nepal", price !== "all" ? priceOptions.find(option => option.value === price)?.label : "Any budget"].join(" · "); const label = customSearchName.trim() || generatedLabel; setSavedSearches(current => [{ id: `${Date.now()}`, label, criteria: currentCriteria }, ...current].slice(0, 8)); setCustomSearchName(""); setSearchNameOpen(false); };
