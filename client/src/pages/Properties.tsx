...
export default function Properties() {
  ...
  const syncedSearches = trpc.sync.savedSearches.useQuery(undefined, { enabled: signedIn });
  const upsertSyncedSearch = trpc.sync.upsertSavedSearch.useMutation();
  const deleteSyncedSearch = trpc.sync.deleteSavedSearch.useMutation();
  ...
  useEffect(() => {
    if (!syncedSearches.data) return;
    const remote = syncedSearches.data.map(row => ({ id: row.searchKey, label: row.label, criteria: (() => { try { return JSON.parse(row.criteria) as SavedSearchCriteria; } catch { return null; } })() })).filter((item): item is SavedSearch => Boolean(item.criteria));
    setSavedSearches(current => { const merged = [...remote, ...current.filter(item => !remote.some(item2 => item2.id === item.id))]; return merged.slice(0, 8); });
  }, [syncedSearches.data]);
  ...
  const saveCurrentSearch = () => { const generatedLabel = [propertyType !== "All" ? propertyType : "Properties", municipality !== "All" ? municipality.replace(" Municipality", "") : location || "Nepal", price !== "all" ? priceOptions.find(option => option.value === price)?.label : "Any budget"].join(" · "); const label = customSearchName.trim() || generatedLabel; const id = `${Date.now()}`; const saved = { id, label, criteria: currentCriteria }; setSavedSearches(current => [saved, ...current.filter(item => item.id !== id)].slice(0, 8)); if (signedIn) upsertSyncedSearch.mutate({ searchKey: id, label, criteria: currentCriteria }); setCustomSearchName(""); setSearchNameOpen(false); };
  ...
  const renameSavedSearch = (saved: SavedSearch) => { const label = editingSavedLabel.trim(); if (!label) return; const renamed = { ...saved, label }; setSavedSearches(current => current.map(item => item.id === saved.id ? renamed : item)); if (signedIn) upsertSyncedSearch.mutate({ searchKey: saved.id, label, criteria: saved.criteria }); setEditingSavedId(null); setEditingSavedLabel(""); };
  const deleteSavedSearch = (saved: SavedSearch) => { setSavedSearches(current => current.filter(item => item.id !== saved.id)); if (signedIn) deleteSyncedSearch.mutate({ searchKey: saved.id }); };
