...
export default function NaturalLanguageSearch() {
  ...
  const remoteHistory = trpc.sync.queryHistory.useQuery(undefined, { enabled: signedIn });
  const upsertQuery = trpc.sync.upsertQuery.useMutation();
  const clearRemoteHistory = trpc.sync.clearQueryHistory.useMutation();
  ...
  useEffect(() => { if (remoteHistory.data) setQueryHistory(current => remoteHistory.data.map(item => item.query).concat(current.filter(item => !remoteHistory.data.some(remote => remote.query.toLowerCase() === item.toLowerCase()))).slice(0, 5)); }, [remoteHistory.data]);
  ...
  function runQuery(nextQuery: string) {
    ...
    if (signedIn) upsertQuery.mutate({ query: normalized });
